...
public class Group implements Voencom {
	...
	private Student[] getStudentArray() {
		Student[] student = new Student[countGroup];
		System.arraycopy(group, 0, student, 0, student.length);
		return student;
	}

	public void saveToFile(File file, Group group) {
		if (file == null && group == null) {
			throw new IllegalArgumentException("Fail with information");
		}
		try (PrintWriter pw = new PrintWriter(file)) {
			for (Student student : group.getStudentArray()) {
				if (student != null) {
					pw.println(student.getName() + "," + student.getSurName() + "," + student.getMiddleName() + ","
							+ student.getAge() + "," + student.isSex() + "," + student.getFaculty() + ","
	...
	public Group loadFromFile(File file) {
		Group nextGroup = new Group();
		try (BufferedReader br = new BufferedReader(new FileReader(file))) {
			String text = "";
			for (; (text = br.readLine()) != null;) {
				String[] arrStudent = text.split(",");
				String name = arrStudent[0];
				String surName = arrStudent[1];
				String middleName = arrStudent[2];
				int age = Integer.valueOf(arrStudent[3]);
				boolean sex = (arrStudent[4].equals("true"));


				Student student = new Student(name, middleName, surName, age, sex);
				nextGroup.addStudents(student);
